...
my $login_url = "https://$opt_h/IGS/includes/up_login.php";
my $request;
my $response;
...
$request = POST "https://$opt_h$upload",
...
if (($response->code() == 302) && ($response->header("Location") =~ m/message=(.*)/)) {
    my $message = $1;
    $message =~ s/<\/?b>//g;
    $message =~ s/<BR\/>/\n/g;
    print "The following file(s) have been uploaded successfully:\n$message\n";
}
